...
from models.db_emp import obtener_conexion, cerrar_conexion
...
##################
def valida_cliente(rut_empresa):
    conn = obtener_conexion()  # Asegúrate de que esto devuelve una conexión
    cursor_cli = None  # Inicializa cur_emp aquí para evitar el UnboundLocalError
    if conn:
        try:
            cursor_cli = conn.cursor()  # Crea un cursor
            cursor_cli.execute("SELECT rut_empresa, fecha_expiracion FROM acceso_empresa WHERE rut_empresa = %s", (rut_empresa,))
            res_valida = cursor_cli.fetchone()

            if res_valida:
                rut_empresa, fecha_expiracion = res_valida
                return {'rut': rut_empresa, 'fecha_expiracion': fecha_expiracion}
            else:
                return None
        except mysql.connector.Error as error:
            print(f"Error en la consulta: {error}")
        finally:
            if cursor_cli:  # Solo intenta cerrar si cursor_cli fue creado
                cursor_cli.close()
...
############## resultado funcion usadno Diccionario (retorno de mas de 2 valores)
        resultado = valida_cliente(request.form['rut_empresa']) 
       ...
       # rut_empresa = rut_cli 
        ...
        if resultado is None:
           ...
           # rut_cli = resultado['rut']
            fecha_clie_expira = resultado['fecha_expiracion']
